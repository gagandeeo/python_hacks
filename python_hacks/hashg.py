...
def ditto_crypt_url(cryptHash, url):
	...
	print("password not in password list")
############# 	SHA224 HASHER ##########
def ditto_sha224_url(sha224hash, url):
	passlist = str(urlopen(url).read(),'utf-8')
	for password in passlist.split('\n'):
		hashguess = hashlib.sha224(bytes(password, 'utf-8')).hexdigest()
		if hashguess == sha224hash:
			print(colored("[+] Password is: " + str(password),'green'))
			return(str(password))
		else:
			print(colored("[-] Password guess " + str(password) + " does not match, trying next....", 'yellow'))
	print("password not in password list")
def ditto_sha224_list(sha224hash, list):
	passlist = list
	for password in passlist:
		hashguess = hashlib.sha224(bytes(password, 'utf-8')).hexdigest()
		if hashguess == sha224hash:
	...
	print("Password not in password list")
############### SHA256 HASHER ##################
def ditto_sha256_url(sha256hash, url):
	passlist = str(urlopen(url).read(),'utf-8')
	for password in passlist.split('\n'):
		...
		if hashguess == sha256hash:
			...
			print(colored("[-] Password guess " + str(password) + " does not match, trying next....", 'yellow'))
	print("password not in password list")
def ditto_sha256_list(sha256hash, list):
	passlist = list
	for password in passlist:
		hashguess = hashlib.sha256(bytes(password, 'utf-8')).hexdigest()
		if hashguess == sha256hash:
	...
	print("Password not in password list")
################### SHA512 HASHER ####################
def ditto_sha512_url(sha512hash, url):
	passlist = str(urlopen(url).read(),'utf-8')
	for password in passlist.split('\n'):
		hashguess = hashlib.sha512(bytes(password, 'utf-8')).hexdigest()
		if hashguess == sha512hash:
			print(colored("[+] Password is: " + str(password),'green'))
			return(str(password))
		else:
			print(colored("[-] Password guess " + str(password) + " does not match, trying next....", 'yellow'))
	print("password not in password list")
def ditto_sha512_list(sha512hash, list):
	passlist = list
	for password in passlist:
		hashguess = hashlib.sha512(bytes(password, 'utf-8')).hexdigest()
		if hashguess == sha512hash:
